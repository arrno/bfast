...
-   [x] Create the `arrno/homebrew-tap` repository (empty repo is fine, GoReleaser will add `Casks/bfast.rb`); grant the release bot push access.
...
-   [ ] Confirm `brew install --cask arrno/tap/bfast` and `scoop install bfast` work on their platforms.
